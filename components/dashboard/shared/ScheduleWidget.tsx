...
export function ScheduleWidget({ subjects }: { subjects: any[] }) {
    ...
    const now = new Date();
    const currentDay = ['DG', 'DL', 'DM', 'DC', 'DJ', 'DV', 'DS'][now.getDay()]; // Mapatge per a abreviatures catalanes de la BD (Dilluns, Dimarts...)
    // O simplified: Just check string inclusion roughly for now if we don't strict parse
