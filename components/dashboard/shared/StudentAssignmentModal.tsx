...
export default function StudentAssignmentModal({ assignment, userId, onClose }: StudentAssignmentModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!content.trim()) {
            setMsg({ type: "error", text: "Si us plau, introdueix el contingut o l'enllaç del teu lliurament." });
            return;
        }
            ...
            <div className="relative bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Tasca: {assignment.title}</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-100 dark:border-zinc-700/50">
                            <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Assignatura</p>
                            <div className="flex items-center gap-2">
                        ...
                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-100 dark:border-zinc-700/50">
                            <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Data límit</p>
                            <div className="flex items-center gap-2 text-zinc-700 dark:text-zinc-300">
                                <Calendar className="w-4 h-4 text-indigo-500" />
                                <span className="text-sm font-medium">
                                    {dueDate.toLocaleDateString("ca-ES", { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })}
                                </span>
                            </div>
                            {assignment.late_due_date && (
                                <p className="text-[10px] text-amber-500 mt-1 font-medium">
                                    Permet retard fins a: {new Date(assignment.late_due_date).toLocaleDateString("ca-ES", { day: 'numeric', month: 'short' })}
                                </p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-2 flex items-center gap-2">
                                <FileText className="w-4 h-4 text-zinc-400" /> Instruccions del professor
                            </h3>
                            <div className="prose prose-sm dark:prose-invert max-w-none bg-zinc-50/50 dark:bg-zinc-800/30 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800">
                                {assignment.description || "No hi ha instruccions addicionals."}
                            </div>
                        </div>
                        ...
                        {assignment.content_url && (
                            <div className="flex items-center justify-between p-4 bg-indigo-50/30 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/20 rounded-xl">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-indigo-900 dark:text-indigo-100">Material de l'activitat</p>
                                        <p className="text-xs text-indigo-600 dark:text-indigo-400">Descarrega els fitxers necessaris per fer la tasca.</p>
                                    </div>
                                </div>
                                <a
                    ...
                    {/* Nota i retorn (estat del lliurament actual) */}
                    {isSubmitted && (
                        <div className={`p-6 rounded-2xl border ${isGraded ? 'bg-emerald-50/30 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/20' : 'bg-blue-50/30 dark:bg-blue-500/5 border-blue-100 dark:border-blue-500/20'}`}>
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-sm font-bold flex items-center gap-2">
                                    ...
                                    {isGraded ? 'Tasca qualificada' : 'Tasca lliurada'}
                                </h3>
                                {isGraded && (
                            ...
                            <div className="space-y-4">
                                ...
                                {isGraded && assignment.feedback && (
                                    <div>
                                        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1.5">Comentaris del professor</p>
                    ...
                    {isReturned && (
                        <div className="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl flex gap-3">
                            ...
                            <div>
                                <p className="text-sm font-bold text-amber-700 dark:text-amber-400">Tasca retornada</p>
                                <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">
                                    El professor ha retornat el teu lliurament. Revisa el feedback i torna a lliurar-lo quan estigui llest.
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-4 border-t border-zinc-100 dark:border-zinc-800 pt-6">
                            <div className="flex items-center justify-between">
                                <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-1 flex items-center gap-2">
                                    <FileUp className="w-4 h-4 text-indigo-500" /> {isReturned ? 'Lliurament nou' : 'Fer el lliurament'}
                                </h3>
                                {isStrictlyLate && (
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-xs font-medium text-zinc-500">Contingut o URL de la tasca</label>
                                    <textarea
                                        value={content}
                                        onChange={e => setContent(e.target.value)}
                                        rows={4}
                                        placeholder="Copia aquí l'enllaç al teu document (Google Docs, PDF, etc.) o escriu la resposta a l'activitat."
                                ...
                                >
                                    ...
                                    {isReturned ? 'Tornar a lliurar' : 'Enviar tasca'}
                                </button>
                            </form>
                        </div>
                    )}
                    ...
                    {isLate && !isSubmitted && (
                        <div className="p-8 text-center bg-rose-50 dark:bg-rose-500/5 rounded-2xl border border-rose-100 dark:border-rose-500/20">
                            ...
                            <p className="text-sm text-rose-600 dark:text-rose-400 mt-2">
                                Ja no es permeten lliuraments per a aquesta activitat. Posa't en contacte amb el teu professor si has tingut algun problema.
                            </p>
                        </div>
                    )}
                </div>
