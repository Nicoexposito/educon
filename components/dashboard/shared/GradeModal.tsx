...
export default function GradeModal({ assignment, onClose }: GradeModalProps) {
    ...
    const handleReturn = () => {
        if (!selectedSubmission) return;
        if (!feedback.trim()) {
            setMsg({ type: "error", text: "Debes escribir un comentario antes de devolver la entrega." });
            return;
        }
        setMsg(null);
        startTransition(async () => {
            ...
            if (result.success) {
                setMsg({ type: "success", text: "Entrega devuelta. El alumno podrá entregar de nuevo." });
                router.refresh();
            } else {
                setMsg({ type: "error", text: result.error || "Error desconocido." });
            }
    ...
    const handleAIGrade = async () => {
        ...
        try {
            ...
            setMsg({ type: "error", text: "Error al contactar con la IA." });
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
                    <div>
                        <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Calificar: {assignment.title}</h2>
                        <p className="text-sm text-zinc-500">{assignment.subject?.name} · {submissions.length} entregas</p>
                    </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-72 border-r border-zinc-200 dark:border-zinc-800 overflow-y-auto shrink-0">
                        <div className="p-3">
                            <p className="text-xs font-semibold uppercase text-zinc-400 px-2 mb-2">Entregas</p>
                            {submissions.length === 0 && (
                                <p className="text-sm text-zinc-400 px-2 py-4">No hay entregas todavía.</p>
                            )}
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-1 mt-1 text-xs text-zinc-400">
                                            ...
                                            {sub.submitted_at
                                                ? new Date(sub.submitted_at).toLocaleDateString("es-ES", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })
                                                : "—"}
                                        </div>
                                    </button>
                                );
                        ...
                        {selectedSubmission ? (
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-3 pb-4 border-b border-zinc-100 dark:border-zinc-800">
                                    ...
                                    <div>
                                        <p className="font-semibold text-zinc-900 dark:text-zinc-100">{selectedSubmission.student?.full_name || "Alumno"}</p>
                                        ...
                                        {selectedSubmission.file_url ? (
                                            selectedSubmission.file_url.startsWith('http') ? (
                                                <a href={selectedSubmission.file_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-indigo-600 hover:text-indigo-700 font-medium underline">
                                                    <FileText className="w-4 h-4" /> Ver archivo entregado
                                                </a>
                                            ) : (
                                                selectedSubmission.file_url
                                            )
                                        ) : "Sin contenido en la entrega."}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 flex items-center gap-1">
                                            <MessageSquare className="w-4 h-4 text-indigo-500" /> Comentario / Feedback
                                        </label>
                                        <textarea
                                            value={feedback}
                                ...
                                {showAI && (
                                    <div className="bg-violet-50 dark:bg-violet-500/5 border border-violet-200 dark:border-violet-500/20 rounded-2xl p-5 space-y-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h3 className="font-semibold text-violet-900 dark:text-violet-200">Calificación Asistida por IA</h3>
                                        </div>

                                        <div className="space-y-1.5">
                                            <label className="text-sm font-medium text-violet-700 dark:text-violet-300">
                                                Criterios de evaluación (rúbricas, esquemas, tablas...)
                                            </label>
                                            <textarea
                                                value={aiCriteria}
                                                onChange={e => setAiCriteria(e.target.value)}
                                                rows={4}
                                                placeholder={"Ejemplo:\n- Contenido (40%): profundidad, exactitud\n- Estructura (30%): organización, claridad\n- Presentación (20%): formato, ortografía\n- Originalidad (10%): aportaciones propias"}
                                        ...
                                        >
                                            ...
                                            {isAILoading ? "Analizando..." : "Generar Estimación"}
                                        </button>
                                        ...
                                        {aiResult && (
                                            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-violet-200 dark:border-violet-500/20 p-4 space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-sm font-medium text-violet-700 dark:text-violet-300">Nota estimada por IA:</span>
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-zinc-400 py-16">
                                ...
                                <p className="font-medium">Selecciona una entrega</p>
                                <p className="text-sm mt-1">Haz click en un alumno de la lista para ver su entrega y calificarla.</p>
                            </div>
                        )}
                    </div>
                </div>
