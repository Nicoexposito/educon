...
export function CreateEventForm({ subjects }: { subjects: { id: string; name: string }[] }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!res.success) {
            setError(res.error || 'Error al crear evento.');
            setLoading(false);
        } else {
            setSuccess(true);
            setTimeout(() => {
    ...
    if (success) {
        return (
            <div className="bg-white dark:bg-zinc-900 rounded-[2rem] p-12 shadow-2xl border border-zinc-200 dark:border-zinc-800 text-center animate-in fade-in zoom-in duration-500">
                ...
                <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-2">¡Evento Creado!</h2>
                <p className="text-zinc-500">El evento ha sido publicado exitosamente para tus alumnos.</p>
            </div>
        );
    }
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <input 
                            name="title" 
                            required 
                            placeholder="Ej. Salida al museo de historia"
                            value={title}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-zinc-700 dark:text-zinc-300 flex items-center gap-2">
                                <BookOpen className="w-4 h-4 text-indigo-500" /> Asignatura (Opcional)
                            </label>
                            <select 
                                name="subject_id" 
                                className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all appearance-none"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-zinc-500 mt-1">Sube una foto que represente este evento.</p>
                        </div>
                    </div>
                </div>
