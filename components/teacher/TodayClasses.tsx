...
import { formatScheduleRange, getClassesForDay, getCurrentClassForSubjects, getTodayName } from "@/lib/academic";

export function TodayClasses({ subjects }: { subjects: any[] }) {
    const now = new Date();
    const currentDay = getTodayName(now);
    const todaysClasses = getClassesForDay(subjects, currentDay);
    const currentClass = getCurrentClassForSubjects(subjects, now);
    const upcomingClasses = todaysClasses.slice(0, 4);
    ...
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col h-full shadow-xs hover:shadow-md transition-shadow">
            <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
                <div>
                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-zinc-500">Horario</p>
                    <h3 className="mt-1 font-bold text-lg text-zinc-900 dark:text-zinc-100">Clases de hoy</h3>
                    <p className="text-sm text-zinc-500">{currentDay}</p>
                </div>
                ...
                {upcomingClasses.length > 0 ? (
                    <div className="space-y-1">
                        {upcomingClasses.map((subj: any, index: number) => (
                             <ClassItem key={`${subj.id}-${subj.activeSchedule?.id || index}`} subject={subj} isActive={currentClass?.id === subj.id && currentClass?.activeSchedule?.start_time === subj.activeSchedule?.start_time} />
                        ))}
                    </div>
                ) : (
            ...
            {upcomingClasses.length > 0 && (
                 <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 border-t border-zinc-100 dark:border-zinc-800">
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-zinc-500">{upcomingClasses.length} clases en el horario de hoy</span>
                        <div className="w-24 h-1.5 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                             <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${Math.min(100, (upcomingClasses.length / 6) * 100)}%` }} />
                        </div>
                    </div>
                </div>
            )}
...
function ClassItem({ subject, isActive }: { subject: any, isActive: boolean }) {
    const scheduleLabel = formatScheduleRange(subject.activeSchedule, subject.schedule || 'Horario no definido');
                 ...
                 <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <span>{subject.student_count ?? subject.students_count ?? 0} alumnos</span>
                        </div>
                    </div>
                 </div>
            </div>
