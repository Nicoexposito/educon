...
export default async function GradesPage() {
    ...
    const allGrades = [
        ...submissions.map((row: any) => ({
            ...
            subject: row.assignment?.subject?.name || "Sin asignatura",
            title: row.assignment?.title || "Trabajo",
        ...
        ...gradeRows.map((row: any) => ({
            id: `grade-${row.id}`,
            subject: row.grade_item?.subject?.name || "Sin asignatura",
            title: row.grade_item?.name || "Calificación",
    ...
    return (
        <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 p-6 md:p-8 max-w-6xl mx-auto">
            <header className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500 mb-2">Ventana de calificaciones</p>
                    <h1 className="text-3xl font-bold tracking-tight">Notas finales</h1>
                    <p className="text-zinc-500 mt-2">Todas tus notas publicadas por asignatura y trabajos corregidos.</p>
                </div>
                <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-6 py-4 shadow-sm">
                    <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Media global</p>
            ...
            <section className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
                {allGrades.length > 0 ? (
                    ...
                    <div className="p-14 text-center text-zinc-500">
                        ...
                        <p className="font-semibold">Todavía no hay calificaciones publicadas.</p>
                    </div>
                )}
            </section>
        </main>
